...
import '../flare/flare_render_box.dart';
import 'flare.dart';
...
class FlareActorRenderObject extends FlareRenderBox {
  ...
  set color(Color value) {
    if (value != _color) {
      _color = value;
      if (_artboard != null) {
        _artboard.overrideColor = value == null ? null : Float32List.fromList([value.red / 255.0, value.green / 255.0, value.blue / 255.0, value.opacity]);
      }
      markNeedsPaint();
    }
  }
  ...
  bool get isPlaying => !_isPaused && ((_controller?.isActive?.value ?? false) || _animationLayers.isNotEmpty);
  ...
  void load() {
    ...
    loadFlare(_filename).then((FlutterActor actor) {
      ...
      FlutterActorArtboard artboard = actor.artboard.makeInstance() as FlutterActorArtboard;
      artboard.initializeGraphics();
      _artboard = artboard;
      _artboard.overrideColor = _color == null ? null : Float32List.fromList([_color.red / 255.0, _color.green / 255.0, _color.blue / 255.0, _color.opacity]);
  ...
  void advance(double elapsedSeconds) {
    if (isPlaying) {
      ...
      for (int i = 0; i < _animationLayers.length; i++) {
        ...
        lastMix = (layer.mixSeconds == null || layer.mixSeconds == 0.0) ? 1.0 : min(1.0, layer.mix / layer.mixSeconds);
    ...
    if (_artboard != null && _controller != null && !_controller.advance(_artboard, elapsedSeconds)) {
