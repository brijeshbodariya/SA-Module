...
const Color _kAppBackgroundColor = const Color(0xff0c0c0c);
...
class _AnimationDemoHomeState extends State<AnimationDemo> {
  ...
  bool _handleScrollNotification(ScrollNotification notification, double midScrollOffset) {
    if (notification.depth == 0 && notification is ScrollUpdateNotification) {
      final ScrollPhysics physics = _scrollController.position.pixels >= midScrollOffset ? const PageScrollPhysics() : const NeverScrollableScrollPhysics();
  ...
  bool _handlePageNotification(ScrollNotification notification, PageController leader, PageController follower) {
    if (notification.depth == 0 && notification is ScrollUpdateNotification) {
      ...
      if (follower.page != leader.page) follower.position.jumpTo(leader.position.pixels);
    }
    return false;
  }
...
class _AllSectionsLayout extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    ...
    final double rowIndicatorWidth = paddedSectionIndicatorWidth + (1.0 - tCollapsed) * (rowTitleWidth - paddedSectionIndicatorWidth);
