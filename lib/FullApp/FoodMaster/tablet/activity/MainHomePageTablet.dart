...
import 'package:smartkit/FullApp/FoodMaster/tablet/helper/drawerstyle/drawerbehavior.dart';
...
import 'package:smartkit/FullApp/FoodMaster/tablet/activity/CartPayment/CartListPageTablet.dart';
import 'package:smartkit/FullApp/FoodMaster/tablet/activity/Order/OrderListTablet.dart';
import 'package:smartkit/FullApp/FoodMaster/tablet/activity/Restaurant/RestaurantListTablet.dart';
...
import 'package:smartkit/FullApp/FoodMaster/tablet/activity/profile/FavouriteListTablet.dart';
import 'package:smartkit/FullApp/FoodMaster/tablet/activity/profile/UserInfoPageTablet.dart';
import 'package:smartkit/SmartkitScreen/Screen/SmartKitHome.dart';

import 'Notification/NotificationListTablet.dart';
import 'authentication/LoginHomePageTablet.dart';
import 'dart:io' show Platform;

Restaurant selectedrestaurant;
Item selecteditem;

class MainHomePageTablet extends StatefulWidget {
  final String from;
  const MainHomePageTablet({
    Key key,
    this.from,
  }) : super(key: key);
  @override
  _MainHomePageStateTablet createState() => _MainHomePageStateTablet();
}

class _MainHomePageStateTablet extends State<MainHomePageTablet> {
  Future<SharedPreferences> _sprefs = SharedPreferences.getInstance();
  int selectedMenuItemId;
  List<MenuItem> menuitems = [];
  HomeScreenPageTablet homepage;
  int counter;
  List<Item> itemlist = [
    Item(
        id: 1,
  ...
  void initState() {
    ...
    homepage = new HomeScreenPageTablet();
    counter = 1;
    getData1();
    setState(() {
      // getData();
        ...
        appBar: AppBar(
            ...
            actions: [
              IconButton(
                  ...
                  color: ColorsRes.IconColor,
                  onPressed: () {
                    Navigator.of(context).push(CupertinoPageRoute(builder: (context) => UserInfoPageTablet()));
                  }),
              ButtonClickAnimation(
                onTap: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) => CartListPageTablet(
                            id: 1,
                            id1: 3.toString(),
                            id2: 1.toString(),
                            name: "Fresh Pasta and Risotto",
                    ...
                    child: GestureDetector(
                        onTap: () {
                          Navigator.pushReplacement(
                            context,
                            MaterialPageRoute(
                                builder: (context) => LoginHomePageTablet(
                                      from: "intro",
                                    )),
                          );
                        },
  ...
  void openSpecifiPage(itemId) {
    if (itemId == 0) {
      ...
      Navigator.of(context).push(CupertinoPageRoute(builder: (context) => OrderListTablet()));
    } else if (itemId == 2) {
      Navigator.of(context).push(CupertinoPageRoute(builder: (context) => AddressListTablet()));
    } else if (itemId == 3) {
      Navigator.of(context).push(CupertinoPageRoute(builder: (context) => NotificationListTablet()));
    } else if (itemId == 4) {
      Navigator.of(context).push(CupertinoPageRoute(builder: (context) => FavouriteListTablet()));
    } else if (itemId == 5) {
      Navigator.of(context).push(CupertinoPageRoute(
          builder: (context) => RestaurantListTablet(
                fromsearch: false,
              )));
    } else if (itemId == 6) {
