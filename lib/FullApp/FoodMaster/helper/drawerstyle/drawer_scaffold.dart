...
class _DrawerScaffoldState<T> extends State<DrawerScaffold> with TickerProviderStateMixin {
  ...
  int get mainDrawerIndex => max(0, widget.drawers.indexWhere((element) => element.direction == widget.mainDrawer));
  @override
  void initState() {
    super.initState();
  ...
  MenuController dcreateController(BuildContext context, SideDrawer d, TickerProvider vsync, Function(double) onAnimated) {
  ...
  assignContoller() {
    if (menuControllers == null)
      ...
      for (var i = 0; i < widget.drawers.length && i < menuControllers.length; i++) {
    ...
    if (widget.appBar != null)
      return AppBar(
          ...
          leading: widget.appBar.leading ??
              new IconButton(
                  icon: CachedNetworkImage(imageUrl: "https://smartkit.wrteam.in/smartkit/foodmaster/button_dropmenu.png"),
                  onPressed: () {
        ...
        : GestureDetector(
            ...
            onHorizontalDragUpdate: (details) {
              ...
              if (isOpening && dx > 0 && dx <= maxSlideAmount) {
                ...
                menuController._animationController.animateTo(percentage, duration: Duration(microseconds: 0));
                menuController._animationController.notifyStatusListeners(AnimationStatus.forward);
              } else if (!isOpening && dx <= 0 && dx >= -maxSlideAmount) {
                percentage = Utils.fixed(1.0 + dx / maxSlideAmount, 3);

                menuController._animationController.animateTo(percentage, duration: Duration(microseconds: 0));
                menuController._animationController.notifyStatusListeners(AnimationStatus.reverse);
              }
            },
  ...
  Widget build(BuildContext context) {
    focusDrawerIndex = min(widget.drawers.length - 1, focusDrawerIndex ?? widget.drawers.length);
    return Stack(
      children: [
...
class MenuController extends ChangeNotifier {
  ...
  MenuController(this._drawer, this.onAnimated, {this.vsync, BuildContext context})
...
class DrawerScaffoldController {
  ...
  bool isOpen([Direction direction = Direction.left]) => _menuControllers.where((element) => element._drawer.direction == direction && element.isOpen()).isNotEmpty;
}
