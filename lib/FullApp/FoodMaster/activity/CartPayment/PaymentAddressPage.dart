...
import 'package:smartkit/FullApp/FoodMaster/activity/CartPayment/PaymentPage.dart';
...
class PaymentAddressPage extends StatefulWidget {
  ...
  final List timeslot;
  const PaymentAddressPage({Key key, this.address, this.name, this.phone, this.timeslot, this.openTime, this.closTome, this.mini, this.id}) : super(key: key);
  @override
  _PaymentAddressState createState() => _PaymentAddressState();
}

class _PaymentAddressState extends State<PaymentAddressPage> {
  ...
  Future<void> payProcees() async {
    ...
    Navigator.of(context).push(CupertinoPageRoute(builder: (context) => PaymentPage(timeslot: widget.timeslot)));
  }
}
