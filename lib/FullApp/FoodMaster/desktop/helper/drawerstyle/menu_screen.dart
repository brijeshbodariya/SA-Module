...
import 'package:smartkit/FullApp/FoodMaster/desktop/helper/drawerstyle/drawer_scaffold.dart';
...
class SideDrawer<T> extends StatefulWidget {
  SideDrawer({
    ...
    this.cornerRadius,
    Key key,
  })  : assert((child != null && menu == null && itemBuilder == null) || (child == null && menu != null)),
        this.percentage = percentage ?? 0.8,
        this.degree = degree == null ? null : max(min(45, degree), 15),
        this.scaleDownCurve = new Interval(0.0, 0.3, curve: curve ?? Curves.easeOut),
        this.scaleUpCurve = new Interval(0.0, 1.0, curve: curve ?? Curves.easeOut),
        this.slideOutCurve = new Interval(0.0, 1.0, curve: curve ?? Curves.easeOut),
        this.slideInCurve = new Interval(0.0, 1.0, curve: curve ?? Curves.easeOut),
        super(key: key);
  ...
  double maxSlideAmount(context) => drawerWidth ?? MediaQuery.of(context).size.width * percentage;
...
class _SideDrawerState extends State<SideDrawer> with TickerProviderStateMixin {
  ...
  setSelectedRenderBox(RenderBox newRenderBox, bool useState) async {
    ...
    final newYTop = newRenderBox.localToGlobal(Offset(0.0, 0.0), ancestor: renderBox).dy;
  ...
  Widget createMenuItems(MenuController menuController) {
    ...
    if (widget.child != null) {
      ...
      final millis = menuController.state != MenuState.closing ? 150 * widget.menu.items.length : 600;

      final maxDuration = (widget.menu.items.length - 1) * perListItemDelay + animationIntervalDuration;
      ...
      for (var i = 0; i < widget.menu.items.length; ++i) {
        ...
        listItems.add(buildListItem(menuController, item, animationIntervalStart, animationIntervalEnd, millis, maxDuration));
      }
    }

    return Container(
      alignment: widget.alignment,
      margin: EdgeInsets.only(left: widget.direction == Direction.left ? 0 : MediaQuery.of(context).size.width - maxSlideAmount),
  ...
  buildListItem(
    ...
    Widget listItem = widget.itemBuilder == null
        ? _MenuListItem(
            ...
            icon: item.icon == null ? null : item.icon, //Icon(item.icon),
            onTap: onTap,
    ...
    if (widget.animation)
      return AnimatedMenuListItem(
        ...
        curve: Interval(animationIntervalStart / maxDuration, animationIntervalEnd / maxDuration, curve: Curves.easeOut),
  ...
  Widget createDrawer(MenuController menuController) {
    ...
    return Transform(
      transform: Matrix4.translationValues(
        widget.slide ? (widget.direction == Direction.left ? 1 : -1) * widget.maxSlideAmount(context) * (controller.slidePercent - 1) : 0,
  ...
  Widget build(BuildContext context) {
    ...
    textStyle = widget?.textStyle ?? Theme.of(context).textTheme.subtitle1.copyWith(color: widget.color.computeLuminance() < 0.5 ? Colors.white : Colors.black);
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      image: new DecorationImage(
                        ...
                        colorFilter: ColorFilter.mode(
                            ColorsRes.bgcolor.withOpacity(0.5),
                            BlendMode.lighten),
                        image: AssetImage(
                          'https://smartkit.wrteam.in/smartkit/foodmaster/background_image.png',
                        ),
                      ),
                    ),*/
                    alignment: Alignment.centerRight,
                    child: Card(
                      ...
                      child: CachedNetworkImage(
                        imageUrl: 'https://smartkit.wrteam.in/smartkit/foodmaster/drawerimage.png',
                        fit: BoxFit.fill,
                        alignment: Alignment.centerRight,
                        width: MediaQuery.of(context).size.width / 3.5,
                        height: MediaQuery.of(context).size.height / 1.5,
                        colorBlendMode: BlendMode.lighten,
                        color: ColorsRes.bgcolor.withOpacity(0.9),
                      ),
                    ),
                  ),
                  createDrawer(menuController),
                  widget.animation && shouldRenderSelector
                      ? ItemSelector(left: widget.direction == Direction.right ? MediaQuery.of(context).size.width - maxSlideAmount : 0, selectorColor: selectorColor, top: actualSelectorYTop, bottom: actualSelectorYBottom, opacity: selectorOpacity)
                      : Container(),
                ],
              ),
            ),
  ...
  static _SideDrawerState of(BuildContext context, {bool nullOk = true}) {
    assert(nullOk != null);
    assert(context != null);
    final _SideDrawerState result = context.findAncestorStateOfType<_SideDrawerState>();
    if (nullOk || result != null) return result;
    throw FlutterError.fromParts(<DiagnosticsNode>[ErrorSummary('_SideDrawerState.of() called with a context that does not contain a _SideDrawerState.'), context.describeElement('The context used was')]);
  }
}
...
class _MenuListItem extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    TextStyle _textStyle = textStyle.copyWith(color: isSelected ? selectorColor : textStyle.color);

    List<Widget> children = [];
    if (icon != null)
      children.add(Padding(
        padding: EdgeInsets.only(right: 12),
        child: IconTheme(data: IconThemeData(color: _textStyle.color), child: icon),
      ));
    children.add(
      Expanded(
        child: Container(
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: drawBorder
            ? ShapeDecoration(
                shape: Border(
                  left: BorderSide(color: isSelected ? selectorColor : Colors.transparent, width: 5.0),
                ),
              )
            : null,
        child: Padding(
