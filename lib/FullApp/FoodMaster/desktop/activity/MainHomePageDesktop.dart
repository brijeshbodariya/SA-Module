...
import 'package:smartkit/FullApp/FoodMaster/desktop/activity/CartPayment/CartListPageDesktop.dart';
import 'package:smartkit/FullApp/FoodMaster/desktop/activity/Order/OrderListDesktop.dart';
import 'package:smartkit/FullApp/FoodMaster/desktop/activity/Restaurant/RestaurantListDesktop.dart';
import 'package:smartkit/FullApp/FoodMaster/desktop/activity/TermsConditionActivityDesktop.dart';
import 'package:smartkit/FullApp/FoodMaster/desktop/activity/home/HomeScreenPageDesktop.dart';
import 'package:smartkit/FullApp/FoodMaster/desktop/activity/profile/AddressListDesktop.dart';
import 'package:smartkit/FullApp/FoodMaster/desktop/activity/profile/FavouriteListDesktop.dart';
import 'package:smartkit/FullApp/FoodMaster/desktop/activity/profile/UserInfoPageDesktop.dart';
...
import 'package:smartkit/FullApp/FoodMaster/desktop/helper/drawerstyle/drawerbehavior.dart';
...
import 'Notification/NotificationListDesktop.dart';
import 'authentication/LoginHomePageDesktop.dart';
import 'dart:io' show Platform;

Restaurant selectedrestaurant;
Item selecteditem;

class MainHomePageDesktop extends StatefulWidget {
  final String from;
  const MainHomePageDesktop({
    Key key,
    this.from,
  }) : super(key: key);
  @override
  _MainHomePageStateDesktop createState() => _MainHomePageStateDesktop();
}

class _MainHomePageStateDesktop extends State<MainHomePageDesktop> {
  Future<SharedPreferences> _sprefs = SharedPreferences.getInstance();
  int selectedMenuItemId;
  List<MenuItem> menuitems = [];
  HomeScreenPageDesktop homepage;
  int counter;
  List<Item> itemlist = [
    Item(
        id: 1,
  ...
  void initState() {
    ...
    homepage = new HomeScreenPageDesktop();
    counter = 1;
    getData1();
    setState(() {
      // getData();
        ...
        appBar: AppBar(
            ...
            actions: [
              IconButton(
                  ...
                  color: ColorsRes.IconColor,
                  onPressed: () {
                    Navigator.of(context).push(CupertinoPageRoute(builder: (context) => UserInfoPageDesktop()));
                  }),
              ButtonClickAnimation(
                onTap: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) => CartListPageDesktop(
                            id: 1,
                            id1: 3.toString(),
                            id2: 1.toString(),
                            name: "Fresh Pasta and Risotto",
                    ...
                    child: GestureDetector(
                        onTap: () {
                          Navigator.pushReplacement(
                            context,
                            MaterialPageRoute(
                                builder: (context) => LoginHomePageDesktop(
                                      from: "intro",
                                    )),
                          );
                        },
  ...
  void openSpecifiPage(itemId) {
    if (itemId == 0) {
      ...
      Navigator.of(context).push(CupertinoPageRoute(builder: (context) => OrderListDesktop()));
    } else if (itemId == 2) {
      Navigator.of(context).push(CupertinoPageRoute(builder: (context) => AddressListDesktop()));
    } else if (itemId == 3) {
      Navigator.of(context).push(CupertinoPageRoute(builder: (context) => NotificationListDesktop()));
    } else if (itemId == 4) {
      Navigator.of(context).push(CupertinoPageRoute(builder: (context) => FavouriteListDesktop()));
    } else if (itemId == 5) {
      ...
      Navigator.of(context).push(CupertinoPageRoute(builder: (context) => TermsConditionActivityDesktop()));
    } else if (itemId == 7) {
      Navigator.of(context).push(CupertinoPageRoute(builder: (context) => TermsConditionActivityDesktop()));
    }
  }
